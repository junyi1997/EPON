...
int small_R=3700;//觸發區間<ADC_V0
int big_R=1000;//觸發區間>ADC_V0
...
{
  for(int j = 0 ; j < k ; j++){
    for(int i = 0 ; i < 7 ; i++){ ADC_V0[i]+=analogRead(pinADC[i]);//讀取ADC01值}
    //讀取ADC02值
    ...
    for (int i=0;i<14;i++){ADC_V0[i]=ADC_V0[i]/k;}//Serial.print(ADC_V0[i]);Serial.print("\t");
  if (ADC_V0[0]<small_R && ADC_V0[0]>big_R){//Do0
    if(state[0]==0){
      state[0]=1;
  ...
  if (ADC_V0[1]<small_R && ADC_V0[1]>big_R){//Re0
    if(state[1]==0){
      state[1]=1;
  ...
  if (ADC_V0[2]<small_R && ADC_V0[2]>big_R){//Mi0
    if(state[2]==0){
      state[2]=1;
  ...
  if (ADC_V0[3]<small_R && ADC_V0[3]>big_R){//Fa0
    if(state[3]==0){
      state[3]=1;
  ...
  if (ADC_V0[4]<small_R && ADC_V0[4]>big_R){//So0
    if(state[4]==0){
      state[4]=1;
  ...
  if (ADC_V0[5]<small_R && ADC_V0[5]>big_R){//La0
    if(state[5]==0){
      state[5]=1;
